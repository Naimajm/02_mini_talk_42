...
/*   Updated: 2024/11/25 22:22:51 by juagomez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minitalk.h"

// EJECUCION -> ./client PID_SERVER "mensaje"
int	main(int argc, char **argv)
{
	...
	if (argc == 3)
	{
		...
		{
			ft_atob(server_pid, message[index]);
			index++;
		}
		//message[index] = '\n'; // separador 
		//ft_atob(server_pid, message[index]); // envio separador		
	}
	else
	{
		ft_putstr_fd("Error\n", 2);
...
void	ft_atob(int server_pid, char character)
{
	int	bits_position;	
	int	bit_value;
	...
	bits_position = 0;	
	while (bits_position < 8)
	{
		bit_value = (0x01 << bits_position);
